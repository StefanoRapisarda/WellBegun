<script lang="ts">
	...
	import { createLearningTrack, updateLearningTrack } from '$lib/api/learningTracks';
	...
	let { onDone, editData }: { onDone: () => void; editData?: LearningTrack } = $props();
	...
	async function handleSubmit(e: SubmitEvent) {
		...
		if (editData) {
			await updateLearningTrack(editData.id, data);
		} else {
			const created = await createLearningTrack(data);
			for (const tagId of selectedTagIds) {
				await attachTag(tagId, 'learningtrack', created.id);
			}
		}
...
<form onsubmit={handleSubmit} class="form">
	...
	{#if !editData}
		<DefaultTagSuggestions category="learningtrack" bind:selectedTagIds />
	{/if}
