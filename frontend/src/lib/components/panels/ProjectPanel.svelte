<script lang="ts">
	...
	async function handleCreate(projectId: number) {
		...
		const lastUsed = getLastUsedTags('project', activeProjectsList, activeActivitiesList);
			...
			<div class="item" class:is-archived={project.is_archived} use:selectable={{ entityType: 'project', entityId: project.id }}>
				...
				<div class="item-card" ondblclick={() => selectEntity('project', project.id, project.title)} role="button" tabindex="-1">
					<div class="item-header">
						...
						<span class="status-badge">{formatStatus(project.status)}</span>
					</div>
...
<style>
	...
	.status-badge { font-size: 0.7rem; padding: 2px 6px; background: #e5e7eb; border-radius: 4px; color: #374151; }
