<script lang="ts">
	...
	import { deleteActivity, updateActivity, activateActivity, deactivateActivity, archiveActivity } from '$lib/api/activities';
	...
	const STATUS_CYCLE = ['todo', 'in_progress', 'done', 'on_hold', 'cancelled'];
			...
			<div class="item" class:is-archived={activity.is_archived} use:selectable={{ entityType: 'activity', entityId: activity.id }}>
				...
				<div class="item-card" ondblclick={() => selectEntity('activity', activity.id, activity.title)} role="button" tabindex="-1">
					<div class="item-header">
						...
						{#if activity.status}
							<button class="status-btn" class:status-todo={activity.status === 'todo'} class:status-done={activity.status === 'done'} class:status-in-progress={activity.status === 'in_progress'} class:status-on-hold={activity.status === 'on_hold'} class:status-cancelled={activity.status === 'cancelled'} onclick={(e) => { e.stopPropagation(); cycleStatus(activity); }}>{activity.status.replace('_', ' ')}</button>
						{/if}
						{#if activity.duration}
							<span class="duration-badge">{activity.duration} min</span>
						{/if}
						{#if activity.activity_date}
							<span class="date-badge">{new Date(activity.activity_date).toLocaleDateString()}</span>
						{/if}
					</div>
...
<style>
	...
	.status-btn.status-todo { background: #f3f4f6; color: #9ca3af; }
